...
class PlayState extends MusicBeatState
{
	...
	//mod
	var m4:FlxSprite;
	var magiclow:FlxSprite;	
	var magicupper:FlxSprite;
	var shopping:FlxSprite;
	var bshopping:FlxSprite;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'mana':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/mana/manaDialogue'));
			case 'arcana':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/arcana/arcanaDialogue'));
			case 'euphoria':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/euphoria/euphoriaDialogue'));
		}
		...
		if (SONG.stage == null)
		{
			switch (storyWeek)
			{
				...
				case 7:
					if (songLowercase == 'euphoria')
					{
						stageCheck = 'euphoriaStage';
					}
					else if (songLowercase == 'mana')
					{
						stageCheck = 'manaStage';
					}
					else
					{
						stageCheck = 'arcanaStage';
					}
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				...
				case 'manaStage':
				{
					...
					var storewall:FlxSprite = new FlxSprite(-1000, -900).loadGraphic(Paths.image('storewall'));
					storewall.antialiasing = true;
					storewall.scrollFactor.set(0.8, 0.8);
					storewall.active = false;
					add(storewall);
					
					var upstorewall:FlxSprite = new FlxSprite(-1000, -900).loadGraphic(Paths.image('upstorewall'));
					upstorewall.antialiasing = true;
					upstorewall.scrollFactor.set(0.8, 0.8);
					upstorewall.active = false;
					add(upstorewall);			
					
					var shoppingcarts:FlxSprite = new FlxSprite(-1000, -900).loadGraphic(Paths.image('shoppingcart'));
					shoppingcarts.antialiasing = true;
					shoppingcarts.scrollFactor.set(0.9, 0.9);
					shoppingcarts.active = false;
					add(shoppingcarts);
				...
				case 'euphoriaStage':
				{
					...
					curStage = 'euphoriaStage';
					
					var storewall:FlxSprite = new FlxSprite(-1000, -900).loadGraphic(Paths.image('storewall_night'));
					storewall.antialiasing = true;
					storewall.scrollFactor.set(0.8, 0.8);
					storewall.active = false;
					add(storewall);

					var upstorewall:FlxSprite = new FlxSprite(-1000, -900).loadGraphic(Paths.image('upstorewall_night'));
					upstorewall.antialiasing = true;
					upstorewall.scrollFactor.set(0.8, 0.8);
					upstorewall.active = false;
					add(upstorewall);						

					magicupper = new FlxSprite(-1150, -900);
					magicupper.frames = Paths.getSparrowAtlas('magicupper');
					magicupper.animation.addByPrefix('anim', 'anim', 24, true);
					magicupper.animation.play('anim', true);
					magicupper.updateHitbox();
					magicupper.antialiasing = true;
					magicupper.scrollFactor.set(0.8, 0.8);
					magicupper.active = true;
					magicupper.alpha = 0;
					add(magicupper);

					var above:FlxSprite = new FlxSprite(-1000, -900).loadGraphic(Paths.image('overlay'));
					above.antialiasing = true;
					above.scrollFactor.set(0.8, 0.8);
					above.active = false;
					add(above);

					m4 = new FlxSprite(-600, -1550).loadGraphic(Paths.image('m4'));
					m4.antialiasing = true;
					m4.setGraphicSize(Std.int(m4.width * 0.5));
					m4.scrollFactor.set(0.85, 0.85);
					m4.active = false;
					m4.alpha = 100;
					add(m4);
					
					magiclow = new FlxSprite(-1150, -900);
					magiclow.frames = Paths.getSparrowAtlas('magiclow');
					magiclow.animation.addByPrefix('anim', 'anim', 24, true);
					magiclow.animation.play('anim', true);
					magiclow.updateHitbox();
					magiclow.antialiasing = true;
					magiclow.scrollFactor.set(0.8, 0.8);
					magiclow.active = true;
					magiclow.alpha = 0;
					add(magiclow);
					
					shopping = new FlxSprite(-500, -20);
					shopping.frames = Paths.getSparrowAtlas('shopping_anim');
					shopping.animation.addByPrefix('anim', 'anim', 13, true);
					shopping.animation.play('anim', true);
					shopping.updateHitbox();
					shopping.antialiasing = true;
					shopping.scrollFactor.set(0.9, 0.9);
					shopping.active = true;
					add(shopping);
					
					bshopping = new FlxSprite(1630, 50);
					bshopping.frames = Paths.getSparrowAtlas('shopping_anim');
					bshopping.animation.addByPrefix('anim', 'anim', 13, true);
					bshopping.animation.play('anim', true);
					bshopping.setGraphicSize(Std.int(bshopping.width * -1));
					//bshopping.Xflip = true;
					bshopping.updateHitbox();
					bshopping.antialiasing = true;
					bshopping.scrollFactor.set(0.9, 0.9);
					bshopping.active = true;
					add(bshopping);
	...
	function euphoriaStage(?dialogueBox:DialogueBox):Void
	{
		var black:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		black.scrollFactor.set();
		add(black);
		
		dad.playAnim('cutsceneA');
		camHUD.visible = false;
		
		//Plate
		dad.visible = true;
		
		new FlxTimer().start(0.3, function(TimerA:FlxTimer)
		{
			black.alpha -= 0.15;
		...
		{
		new FlxTimer().start(1, function(TimerB:FlxTimer)
			{
		//Camera
		camFollow.x -= 50;
		camFollow.y += 50;
		...
		FlxTween.tween(FlxG.camera, {zoom: 1.0}, 2);
			...
			{
				inCutscene = true;
				new FlxTimer().start(4,function(TimerC:FlxTimer)
				{
					new FlxTimer().start(2,function(TimerD:FlxTimer){
	...
	function arcanaStage(?dialogueBox:DialogueBox):Void
	{
		var black:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		black.scrollFactor.set();
		add(black);
	...
	function manaStage(?dialogueBox:DialogueBox):Void
	{
		...
		//Pain
		new FlxTimer().start(1, function(zeroTimer:FlxTimer)
		{
			//Camera 
			camFollow.x -= 50;
			camFollow.y += 50;
			...
			new FlxTimer().start(0.3, function(oneTimer:FlxTimer)
			{
				dad.alpha += 0.15;
				...
				{
					new FlxTimer().start(1,function(twoTimer:FlxTimer)
					{
						if (dialogueBox != null)
						{
							new FlxTimer().start(0.5,function(threeTimer:FlxTimer)
							{
								new FlxTimer().start(0.1, function(newerTimer:FlxTimer)
								{
									...
									else
									{
										camHUD.visible = true;
										add(dialogueBox);
										new FlxTimer().start(2,function(fourTimer:FlxTimer){
											FlxTween.tween(FlxG.camera, {zoom:defaultCamZoom}, 2);
										});
									}
								});
						...
						{
							new FlxTimer().start(3,function(fiveTimer:FlxTimer)
								{
									startCountdown();
								});
						}
